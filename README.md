...
- **Пустой вишлист:** владелец видит приглашение «Добавить подарок»; по публичной ссылке — сообщение «В этом списке пока нет подарков» и подсказка заглянуть позже.
...
- **Реалтайм:** при резерве или вкладе все открытые страницы этого вишлиста получают обновление по WebSocket (прогресс и счётчик участников) без перезагрузки. Добавление/удаление/изменение подарков и смена порядка тоже приходят по WebSocket.
- **Автозаполнение по ссылке:** при добавлении подарка можно вставить URL товара и нажать «Подтянуть» — приложение подтянет название, картинку и цену из Open Graph / product-мета (если сайт их отдаёт). Иначе — ручной ввод.

### Продуктовые решения и граничные случаи

- **Удалённый подарок, на который уже скидывались:** при удалении подарка владельцем все резервы и вклады по нему удаляются вместе с записью (CASCADE). Мы не храним «историю» вкладов по удалённым товарам и не возвращаем деньги через приложение — это напоминание владельцу: удалять подарок стоит только если он точно не будет куплен, и договорённости с друзьями — вне приложения.
- **Подарок без цены:** можно добавить подарок без указания цены (только название и ссылка). На публичной странице такой подарок можно зарезервировать «полностью» (гость сам решает, что это значит), но сбор по нему не показывается.
- **Один подарок — один полный резерв:** полное резервирование на всю сумму блокирует подарок для других; скинуться можно только пока подарок не зарезервирован полностью.
